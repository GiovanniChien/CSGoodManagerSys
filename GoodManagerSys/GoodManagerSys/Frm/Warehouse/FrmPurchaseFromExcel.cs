...
namespace GoodManagerSys.Frm.Warehouse
{
    public partial class FrmPurchaseFromExcel : Form
    {
        ...
        private void BtnBrowse_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Filter = "excel文件(*.xls,*.xlsx)|*.xls;*.xlsx|excel03文件(*.xls)|*.xls|excel07文件(*.xlsx)|*.xlsx";
                    ...
                    if (row != null)
                    {
                        ...
                        if (good!=null)
                        {
                            ...
                            DgvGoodFromExcel.Rows.Add(new object[]
                            {
                                ...
                                good.Good.Price,
                                good.Count,
                                EStateToString(good.Good.State),
                                ""
                            });
                        }
                        else
        ...
        {
            if (s.Equals("未上架")) return EState.ePrePutaway;
            return EState.eUnsaled;
        }

        private string EStateToString(EState state)
        {
            if (state == EState.ePrePutaway) return "未上架";
            return "未销售";
        }
    }
}
